...
注意：如果你在这里多次插入了“Jonathan Harker”，你将会得到多个名为“Jonathan Harker”的 `Person` 对象。你可能觉得这不太合理，但我们暂时先允许这样做，我们将会在[第七章](../chapter7/index.md)中学习如何限制数据库中不出现多个使用相同姓名的 `Person`。
